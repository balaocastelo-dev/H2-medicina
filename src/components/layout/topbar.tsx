...
import { BuscaDePaciente } from '@/components/guide/busca-paciente';
import { BotaoAjuda } from '@/components/guide/guia-provider';
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 lg:px-6">
      <div className="min-w-0 pl-10 lg:pl-0">
        <p className="truncate text-sm font-medium text-slate-800">{tenantName}</p>
      </div>

      <div className="flex items-center gap-2">
        {podeBuscarPaciente && <BuscaDePaciente />}
        <BotaoAjuda />
      </div>
